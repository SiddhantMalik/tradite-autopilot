...
4. Net out market & sector beta. A move in line with index x beta is not stock-specific. Macro/flow news (RBI rates, crude, INR, FII/DII, global cues/GIFT Nifty) is SECTOR-level, not a single-stock signal. MEASURED (our Marketaux news study): ~80% of entity-tagged items are index/market commentary ("Sensex up/down N pts") that carries NO idiosyncratic forward effect — positive and negative market tone both averaged ~+0.6%/20d. ONLY company-specific items (headline names the company) moved the single stock. So when a headline is just market/sector commentary tagged to the name, treat it as beta (≈neutral for the stock); weight company-specific news heavily.
